...
//Constructor
sym::sym(string n, string t, symtype *ptr, int width): name(n){
...
//Constructor
symtype::symtype(string t, symtype *ptr, int width):
    type(t),
    ptr(ptr),
    width(width) {};
    
//Constructor  
symtable::symtable(string n): name(n), count(0) {}
...
void symtable::print(){
    ls<symtable*> tablelist;
    cout << setw(156) << setfill('=') << "=" << endl;
    ...
    cout << setw(156) << setfill('-') << "-" << setfill(' ') << endl;
    cout << endl;
    for (lsti it = tablelist.begin(); it != tablelist.end();++it)
        (*it)->print();
}
...
void quad::print(){
    if (op == "LEFTOP") cout << result << " = " << arg1 << " << " << arg2;
...
void backpatch(ls<int> l, int addr){
    stringstream strs;
    strs << addr;
    string temp_str = strs.str();
    char *intStr = (char*) temp_str.c_str();
    string str = string(intStr); 
    for (ls<int>::iterator it = l.begin(); it != l.end(); it++) 
        q.Array[*it].result = str;
}

ls<int> makelist(int i){
    ls<int> l(1, i); 
    return l; 
}

ls<int> merge(ls<int> &a, ls<int> &b){
    a.merge(b); 
    return a;   
}
...
bool typecheck(sym* &s1, sym* &s2){
    ...
    else if (s1 = conv(s1, t2->type)) return true;
    else if (s2 = conv(s2, t1->type)) return true;
    else return false;
}
...
int size_type(symtype *t){
    ...
    else if (t->type == "DOUBLE") return 8;
    else if (t->type == "INTEGER") return 4;
    return -1;
}
