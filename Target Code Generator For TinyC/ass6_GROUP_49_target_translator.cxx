/* Target Translator for tinyC (Compiler Lab) */
/* Group - 49 */
...
string asmfilename = "ass6_GROUP_49_";					
string inputfile = "ass6_GROUP_49_test";					

void computeActivationRecord(symtable* st){						// prepares the activation table for a given symtable 
	int param = -20;
	int local = -24;
...
void genasm(){
	...
	for(std::map<int,int>::iterator it = labelMap.begin(); it != labelMap.end(); ++it){
		it->second = count++;														// map from quad number to label number
	}
	list<symtable*> tablelist;
	
	for(list <sym>::iterator it = globalTable->table.begin(); it != globalTable->table.end(); it++) {
		if(it->nested != NULL) tablelist.push_back (it->nested); 					// flatten the global symbol table
	}
	
	for(list<symtable*>::iterator iterator = tablelist.begin(); iterator != tablelist.end(); ++iterator) {
		computeActivationRecord(*iterator);						// compute activation record for all (nested) the symbol table					
	}
	...
	for(vector<quad>::iterator it = Array.begin(); it != Array.end(); it++){
		...
		if(op == "PARAM"){ 										// ifparam -> add to the param list
			...
			else if(op == "SUB"){	 								// subtraction operation
				sfile << "movl \t" << table->ar[arg1] << "(%rbp), " << "%eax" << endl;
				sfile << "\tmovl \t" << table->ar[arg2] << "(%rbp), " << "%edx" << endl;
				sfile << "\tsubl \t%edx, %eax\n";
				sfile << "\tmovl \t%eax, " << table->ar[result] << "(%rbp)";
			...
			else if(op == "MULT"){									// multiplcation operator
				sfile << "movl \t" << table->ar[arg1] << "(%rbp), " << "%eax" << endl;
				bool flag = true;
				if(s.empty() || ((!isdigit(s[0])) && (s[0] != '-') && (s[0] != '+'))) flag = false ;
				else{
			...
			else if(op == "DIVIDE"){								// divide operation
				sfile << "movl \t" << table->ar[arg1] << "(%rbp), " << "%eax" << endl;
				sfile << "\tcltd" << endl;
			...
			// Bit Operators and shift operations (ignored)
			...
			// call a function
				...
				for (list <sym>::iterator it = t->table.begin(); it != t->table.end(); it++){
					...
					if(it->category == "param"){
						if(j == 0){									// the first parameter to the function
							sfile << "movl \t" << table->ar[params[i]] << "(%rbp), " << "%eax" << endl;
							sfile << "\tmovq \t" << table->ar[params[i]] << "(%rbp), " << "%rdi" << endl;
							j++;
						}
						else if(j == 1){							// the second parameter to the function
							sfile << "movl \t" << table->ar[params[i]] << "(%rbp), " << "%eax" << endl;
							sfile << "\tmovq \t" << table->ar[params[i]] << "(%rbp), " << "%rsi" << endl;
							j++;
						}
						else if(j == 2){							// the third parameter to the function
							sfile << "movl \t" << table->ar[params[i]] << "(%rbp), " << "%eax" << endl;
							sfile << "\tmovq \t" << table->ar[params[i]] << "(%rbp), " << "%rdx" << endl;
							j++;
						}
						else if(j == 3){							// the fourth parameter to the function
							sfile << "movl \t" << table->ar[params[i]] << "(%rbp), " << "%eax" << endl;
			...
			else if(op == "FUNC"){									// prologue of a function
				...
				symtable* t = table;								// Function Table
				int i = 0;
				for (list <sym>::iterator it = t->table.begin(); it != t->table.end(); it++){
					if(it->category == "param"){
						if(i == 0){
	...
	// footnote
	sfile << "\t.ident\t	\"Compiled by GROUP_49\"\n";
